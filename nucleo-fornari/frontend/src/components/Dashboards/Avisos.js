    ...
    return (
        <section className="w-full flex items-center flex-col px-20 py-4 overflow-y-scroll h-90vh">
            {avisosData.map((aviso, index) => (
                <div key={index} className="bg-blue-pastel p-10 mb-3 rounded-lg w-4/5 flex flex-col gap-2">
                    <h1 className="text-4xl font-semibold">{aviso.titulo}</h1>
                    <p className="text-2xl py-5">{aviso.conteudo}</p>
                    <h3 className="italic font-semibold">Por {aviso.autor},</h3>
                    <p className="italic">{aviso.dataCriacao}</p>
                </div>
            ))}
        </section>
    );
