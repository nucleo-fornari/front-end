...
const Calendario = ({
  ...
  const generateCalendar = useMemo(() => {
    ...
    const calendar = calendarDays.map(
      ({ month, day, isPlaceholder }, index) => {
        const isNewMonth =
        ...
        return (
          ...
          >
            {!isPlaceholder && eventsForDay.length > 0 ? (
              <span
                className={`w-full h-fit flex flex-col items-start text-xs lg:text-base ${
                  month < 0 ? "text-slate-400" : "text-slate-800"
                }`}
              >
                <p
                  className={`${
                    isToday ? "bg-blue-500 font-semibold text-white-ice" : ""
                  } px-2 rounded-full`}
                >
                  {day}
                </p>
              ...
              <span
                className={`w-full h-fit flex justify-between rounded-full text-xs lg:text-base ${
                  month < 0 ? "text-slate-400" : "text-slate-800"
                } px-2 rounded-full`}
              >
                <p
          ...
          <div className="py-2 flex w-full flex-wrap items-center justify-between gap-6">
            <div className="flex flex-wrap gap-2 sm:gap-3">
              ...
              <button
                ...
                type="button"
                className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-900 hover:bg-gray-100 sm:rounded-xl lg:px-5 lg:py-2.5"
              >
                Hoje
              </button>
            </div>
            <div className="flex w-fit items-center justify-between">
              ...
              >
                <svg
                  className="h-6 w-6 text-gray-800 transition-transform duration-200 ease-in-out hover:text-cyan-500"
              ...
              >
                <svg
                  className="h-6 w-6 text-slate-800 transition-transform duration-200 ease-in-out hover:text-cyan-500"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
