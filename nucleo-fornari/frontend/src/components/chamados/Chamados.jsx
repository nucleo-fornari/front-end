...
const ChamadosSecretaria = () => {
  ...
  const loadTiposChamados = React.useCallback(() => {
  ...
  const isBigger = React.useCallback((v1, v2) => {
    if (typeof v1 == 'string') return v1.localeCompare(v2) > 0;
    if (typeof v2 == "number") return v1 > v2;
  }, []);

  const isSmaller = React.useCallback((v1, v2) => {
    if (typeof v1 == 'string') return v1.localeCompare(v2) < 0;
    if (typeof v2 == "number") return v1 < v2;
  }, []);

  const quickSort = React.useCallback((v, indInicio, indFim) => {
    let j = indFim;
    let i = indInicio;
  ...
  const callOrdenator = React.useCallback((type) => {
    if (data.length < 1) {
      return;
    }
    let v;
  ...
  }, [data, callOrdenator, ordenated]);

  useEffect(() => {
    callOrdenator(parseInt(selectValue));
  }, [selectValue, callOrdenator]);

  useEffect(() => {
    if (isModalGerenciarTiposOpen)
      loadTiposChamados();
  }, [isModalGerenciarTiposOpen, loadTiposChamados])
      ...
      <Box sx={{ p: 5 }}>
        ...
        <div className='flex flex-row items-center justify-between'>
          ...
          <div className='flex flex-row gap-2 items-center'>
            <p>
              <strong>Gerenciar Tipos de Chamado</strong>
            </p>
            <IconButton
